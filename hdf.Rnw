...
<<label=filter,echo=False,fig=FALSE,cache=True,eval=False>>=
fname <- '~/data/hdf/all.qnorm.txt'
dat <- read.delim(fname, sep="|")
...
# NHK conditioned medium
tpoint <- c(0,1,2,3,4,6,8,10,12,18,24)
for (i in 1:length(tpoint)) {
    if (i == 1)
        col.label <- grep('C0_.+mean', names(dat))
    else
        col.label <- c(col.label, grep(paste('N',tpoint[i],'_.+mean',sep=''), 
            names(dat)))
}
ts <- dat[,col.label]
ts <- log2(ts/ts[,1])
rownames(ts) <- dat$Symbol
write.table(ts, paste('~/data/hdf/hdf_nhk_',nrow(ts),'.tsv',sep=''), quote=F, 
    sep='\t', col.names=tpoint)

# control
tpoint <- c(0,2,4,8,12,24)
for (i in 1:length(tpoint)) {
    if (i == 1)
        col.label <- grep('C0_.+mean', names(dat))
    else
        col.label <- c(col.label, grep(paste('C',tpoint[i],'_.+mean',sep=''), 
            names(dat)))
}
ts <- dat[,col.label]
ts <- log2(ts/ts[,1])
rownames(ts) <- dat$Symbol
write.table(ts, paste('~/data/hdf/hdf_ctrl_',nrow(ts),'.tsv',sep=''), quote=F, 
    sep='\t', col.names=tpoint)
...
<<label=logistic,echo=False,fig=False,eval=False>>=
# logistic fit
library(minpack.lm)
...
gof <- function(ts) {
    ...
    ## plot model evaluated at final parameter estimates
    fitted <- logistic(as.list(coef(nls.out)), tpoint)
    p <- pchisq(sum((fitted-as.numeric(ts))^2), length(tpoint)-4)
}
...
pval <- read.delim('~/data/hdf/mds/pval_hdf_nhk_16091.dat', header=F)
ctrl <- ctrl[order(pval),]
treated <- treated[order(pval),]
symbol <- ctrl$Gene
#ctrl$Gene <- paste(1:nrow(ctrl), '. ', ctrl$Gene, sep='')
#treated$Gene <- paste(1:nrow(treated), '. ', treated$Gene, sep='')
names(ctrl) <- c('Gene',0,2,4,8,12,24)
names(treated) <- c('Gene',0,1,2,3,4,6,8,10,12,18,24)
@
...
%\begin{figure}[ht]
%\centering
<<label=top_mfpt,echo=False,fig=True,width=8,height=5,eval=False>>=
...
%\begin{figure}[ht]
%\centering
<<label=fit_ex,echo=False,fig=True,width=7,height=7,eval=True>>=
#idx <- symbol == 'CEBPD'
...
treated <- read.delim('~/data/hdf/hdf_nhk_16091.tsv')
treated.tpoint <- c(0,1,2,3,4,6,8,10,12,18,24)
treated.df <- list(dat=treated[which(treated$Gene=='CCL5'),], cond='NHK', 
    tpoint=treated.tpoint)
#df.ex <- plot.logistic.example(treated.df, 'CCL5')
symbol <- treated.df$dat$Gene
df.treated <- melt(treated.df$dat)
df.treated <- transform(df.treated, cond=treated.df$cond)
df.treated <- transform(df.treated, time=treated.df$tpoint)
df <- df.treated

# logistic fit
idx.treated <- which(names(treated.df$dat)=='Gene')
fit.treated <- apply(treated.df$dat[,-idx.treated],1,function(x) fit.logistic(treated.df$tpoint,x))
colnames(fit.treated) <- symbol

# get upregulation time
nfit <- 100
xfit <- seq(0,max(treated.df$tpoint),length=nfit)
