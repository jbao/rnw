...
<<label=limma,echo=False,fig=True,eval=False>>=
library(limma)
...
res <- hist(-log10(res.limma$adj.P.Val), plot=F)
...
print(ggplot(df,aes(x,y)) + 
    ...
    labs(x=expression(paste(-log[10]~italic(q),'-value')),
        y='Count',title='Basal differential expression of H838 genes\n') 
)
...
<<label=gage,echo=False,fig=False,cache=True,eval=True>>=
# GAGE
library(gage)
library(gageData)
data(go.gs)
...
mSigDB <- readLines("~/data/MSigDB/c2.all.v3.0.entrez.gmt")
...
<<label=colorbar,echo=False,fig=True,eval=False>>=
mycolor <- colorRampPalette(c('white','#8EC13A'))
# Generate data
pp <- function (n,r=4) {
...
print(ggplot(pp(20), aes(x=x,y=y)) +
    geom_tile(aes(fill=z)) +
    scale_fill_gradientn(colours = mycolor(128)) +
...
<<label=circle,echo=False,fig=True,eval=False>>=
...
dat1 <- circleFun(c(0,1),2,npoints = 100)
dat2 <- circleFun(c(0,2),4,npoints = 100)
dat3 <- circleFun(c(0,3),6,npoints = 100)
dat4 <- circleFun(c(0,4),8,npoints = 100)
dat5 <- circleFun(c(0,5),10,npoints = 100)
#geom_path will do open circles, geom_polygon will do filled circles
print(ggplot(dat1,aes(x,y)) + geom_path() +
    geom_path(data=dat2,aes(x,y)) +
    geom_path(data=dat3,aes(x,y)) +
    geom_path(data=dat4,aes(x,y)) +
    geom_path(data=dat5,aes(x,y)) +
...
<<label=unscratched,echo=False,fig=True,eval=True>>=
cond <- 'Mono'
tpoint.label <- c('30min','1h','2h','3h','4h','5h','10h','15h','24h')
wd <- '/home/jbao/data/hpaec/gage_kegg/'
write.dot <- function(cond, tpoint.label) {
    mat <- dat[,grep('mean',names(dat))]
    ref.idx <- grep(paste(cond,'0h',sep='_'), names(mat))
    samp.idx <- grep(paste(cond,tpoint.label,sep='_'), names(mat))
    #test with direction: either up or down
    geneset <- kegg.gs
    res <- gage(mat, gsets = geneset, ref = ref.idx, samp = samp.idx, 
        compare='as.group', saaTest=gs.tTest, same.dir=F)
    df <- as.data.frame(res$greater)
    df <- subset(df[,c('p.val','set.size')], p.val<0.5)
    adj <- foreach(i=1:nrow(df), .combine='rbind') %:%
        foreach(j=1:nrow(df), .combine='c') %dopar% {
        
        overlap <- length(intersect(geneset[[rownames(df)[i]]], 
            geneset[[rownames(df)[j]]]))    
        len.i <- length(geneset[[rownames(df)[i]]])
        len.j <- length(geneset[[rownames(df)[j]]])
        #if (overlap > len.i*0.25 & overlap > len.j*0.25)
        if (overlap > 50)
            res <- 1
        else
            res <- 0
    ...
    set.size <- df$set.size
    diag(adj) <- 0

    # build a graph out of it
    term.net <- graph.adjacency(adj, mode="undirected", diag=F)
    #if (largest==T) {
    #    #pick the largest sub graph if largest is TRUE
    #    c=clusters(groups.net)
    #    groups.net=subgraph(groups.net,names(mygroups)[which(c$membership==(which.max(c$csize)-1))])
    #}

    mycolor <- colorRampPalette(c('white','#8EC13A'))
    pval.trans <- round(-log10(df$p.val)*100)
    pval.trans <- pval.trans - min(pval.trans) + 1
    cols <- mycolor(max(pval.trans))[pval.trans]
    # make the labels
    #V(term.net)$label <- rownames(df)
    #V(term.net)$label <- sub('GO:[0-9]+\\s','',rownames(df))
    V(term.net)$label <- sub('hsa[0-9]+\\s','',rownames(df))
    V(term.net)$width <- sqrt(set.size)*0.1
    V(term.net)$shape <- 'circle'
    V(term.net)$fixedsize <- 'true'
    V(term.net)$style <- 'filled'
    #V(term.net)$fillcolor <- "#8EC13A"
    V(term.net)$fillcolor <- cols
    V(term.net)$fontsize <- sqrt(set.size)
    V(term.net)$fontname <- 'Helvetica'
    term.net$overlap <- 'scalexy'
    term.net$sep <- '+20'
    term.net$outputorder <- 'edgefirst'
    term.net$splines <- 'true'
    term.net$pad <- 2
    write.graph(term.net, paste(wd,'h838_',cond,'_',tpoint.label,'_gage_kegg.dot',sep=''), 'dot')
}
for (i in 1:length(tpoint.label)) {
    write.dot(cond, tpoint.label[i])
}
for (i in 1:length(tpoint.label)) {
    system(paste('neato ',wd,'h838_',cond,'_',tpoint.label[i],'_gage_kegg.dot -Teps -o ',wd,'h838_',cond,'_',tpoint.label[i],'_gage_kegg.eps',sep=''))
}

# create a colour palette depending on the  score
#   cols=designer.colors(col=c("yellow", "orange", "red"),  n=max(scores) )[scores]

#plot(term.net, layout=layout.auto,
#    vertex.size=set.size*0.05,
#         ,vertex.color=cols
#    vertex.label=labels,
#         ,vertex.label.cex=1
...
<<label=hippie,echo=False,fig=False,cache=True,eval=True>>=
...
<<label=tfbs_sig,echo=False,fig=False,eval=False>>=
...
for (i in 1:length(tf.sig)) {
    ...
    tf.entrez <- rbind(tf.entrez, data.frame(tf.entrez=map$idmap$entrez[match(tf,map$idmap$official)],time=tpoint[i],tf.symbol=as.vector(tf)))
}
...
#    ifnotfound=NA))
receptor <- receptor[receptor %in% map$idmap$official]
...
<<label=tf_heatmap,echo=False,fig=True,height=8,eval=False>>=
tf.all$presence <- 1
final <- reshape(tf.all[,c('tf.symbol','time','presence')], v.names='presence',
    idvar='tf.symbol', timevar='time', direction='wide')
final.m <- melt(final, id.vars='tf.symbol')
final.m$variable <- paste(rep(tpoint, each=nrow(final)),'h')
#final.m[is.na(final.m)] <- 0
final.m <- within(final.m, variable <- factor(variable, levels = paste(tpoint,'h')))
final.m <- within(final.m, tf.symbol <- factor(tf.symbol, levels = sort(as.character(tf.symbol))))
mycolor <- colorRampPalette(c('grey30','black'))
print(ggplot(final.m,aes(variable,tf.symbol)) + 
    geom_tile(aes(fill=as.character(value))) +
    scale_fill_manual(values='black',na.value='grey80') +
    #guides(fill = guide_colorbar(barwidth = 0.5, barheight = 10, 
    #    title.hjust=1.6, title.vjust=0.4,
    #    #title.position='right',
    #    title=expression(paste(log[2]~'fold expression')))) +
    labs(x = "",y = "",title='Enriched TFs in H838+HPAEC\n') + 
...
<<label=top_mfpt,echo=False,fig=True,width=8,height=8,eval=True>>=
...
df <- ddply(mfpt.sp, .(receptor,time), summarise, inv.mfpt=1/mean(mfpt),
    inv.sp=1/mean(sp), tf.num=length(mfpt))

print(ggplot(df,aes(receptor,0)) + #geom_point() +
...
<<label=receptor_ts_ex,echo=False,fig=True,width=8,height=4,eval=True>>=
df.subset <- subset(df, receptor=='EGFR')
df.ex <- melt(df.subset, measure.vars=c('inv.mfpt','inv.sp'))
df.ex$variable <- c(rep('Inverse MFPT',nrow(df.subset)),
    rep('Inverse SP',nrow(df.subset)))
print(ggplot(df.ex,aes(time,value)) + geom_point() +
    geom_line() + 
    geom_ribbon(aes(ymin=0,ymax=value),alpha=0.2) +
    facet_wrap(~variable,scales='free_y') +
    labs(x='Time (h)',y='Distance (EGFR -> TFs)')
)
@

% ----------------------------------------------------------------------------

<<label=sp_bootstrap,echo=False,fig=False,cache=True,eval=True>>=
ppi <- 'hippie'
...
#sp.all <- NULL
#for (k in 1:1) {
mean.inv.sp <- function(receptor, tf.num) {
    tf.bootstrap <- sample(tf.access, tf.num)
    #all.inv <- rep(0,length(tf.bootstrap))
    #for (i in 1:length(tf.bootstrap)) {
    #    receptor.entrez <- map$idmap$entrez[which(map$idmap$official==receptor)]
    #    tf.entrez <- map$idmap$entrez[which(map$idmap$official==tf.bootstrap[i])]
    #    receptor.idx <- match(receptor.entrez, V(net)$gene)
    #    tf.idx <- match(tf.entrez, V(net)$gene)
    #    all.inv[i] <- shortest.paths(net, receptor.idx, tf.idx, weights=NA, mode='out')
    #}
    receptor.entrez <- map$idmap$entrez[which(map$idmap$official==receptor)]
    receptor.idx <- match(receptor.entrez, V(net)$gene)
    sp.all <- shortest.paths(net, receptor.idx, weights=NA, mode='out')
    sp.bootstrap <- sp.all[,tf.bootstrap]
    1/mean(sp.bootstrap)
}
mean.inv.mfpt <- function(receptor, tf.num) {
    tf.bootstrap <- sample(tf.access, tf.num)
    receptor.entrez <- map$idmap$entrez[which(map$idmap$official==receptor)]
    tf.entrez <- map$idmap$entrez[match(tf.bootstrap,map$idmap$official)]
    ii <- map$idmap$vertex[which(receptor.entrez == map$idmap$entrez)] + 1
    jj <- map$idmap$vertex[match(tf.entrez,map$idmap$entrez)] + 1
    mfpt.bootstrap <- mat$mfpt[ii,jj]
    1/mean(mfpt.bootstrap)
}
#stop()
sp.bootstrap.all <- foreach(k=1:1000, .combine='cbind') %dopar% {
    ...
    df.bootstrap <- ddply(df, .(receptor,time), function(x) mean.inv.sp(x$receptor,x$tf.num))
    df.integral.bootstrap <- ddply(df.bootstrap, .(receptor), function(x) trapz(sort(x$time),x$V1[order(x$time)]))
    df.integral.bootstrap$V1
}
mfpt.bootstrap.all <- foreach(k=1:1000, .combine='cbind') %dopar% {
    df.bootstrap <- ddply(df, .(receptor,time), function(x) mean.inv.mfpt(x$receptor,x$tf.num))
    df.integral.bootstrap <- ddply(df.bootstrap, .(receptor), function(x) trapz(sort(x$time),x$V1[order(x$time)]))
    df.integral.bootstrap$V1
}
df.integral.sp <- ddply(df, .(receptor), function(x) trapz(sort(x$time),x$inv.sp[order(x$time)]))
df.integral.sp$pval <- rowSums(df.integral.sp$V1<sp.bootstrap.all) / 1000
df.integral.mfpt <- ddply(df, .(receptor), function(x) trapz(sort(x$time),x$inv.mfpt[order(x$time)]))
df.integral.mfpt$pval <- rowSums(df.integral.mfpt$V1<mfpt.bootstrap.all) / 1000
...
<<label=receptor_pval_ex,echo=False,fig=True,width=8,height=4,eval=True>>=
idx <- which(unique(df$receptor)=='EGFR')
df.ex <- data.frame(type='Inverse MFPT', value=mfpt.bootstrap.all[idx,])
df.ex <- rbind(df.ex, data.frame(type='Inverse SP', value=sp.bootstrap.all[idx,]))
df.cutoff <- data.frame(type='Inverse MFPT', value=df.integral.mfpt$V1[df.integral.mfpt$receptor=='EGFR'],y=max(density(df.ex$value[df.ex$type=='Inverse MFPT'])$y),label='EGFR')
df.cutoff <- rbind(df.cutoff, data.frame(type='Inverse SP', value=df.integral.sp$V1[df.integral.sp$receptor=='EGFR'],y=max(density(df.ex$value[df.ex$type=='Inverse SP'])$y),label='EGFR'))
mfpt.dens <- density(df.ex$value[df.ex$type=='Inverse MFPT'])
sp.dens <- density(df.ex$value[df.ex$type=='Inverse SP'])
df.dens <- data.frame(type='Inverse MFPT', x=mfpt.dens$x[mfpt.dens$x>df.cutoff$value[df.cutoff$type=='Inverse MFPT']],y=mfpt.dens$y[mfpt.dens$x>df.cutoff$value[df.cutoff$type=='Inverse MFPT']])
df.dens <- rbind(df.dens, data.frame(type='Inverse SP', x=sp.dens$x[sp.dens$x>df.cutoff$value[df.cutoff$type=='Inverse SP']],y=sp.dens$y[sp.dens$x>df.cutoff$value[df.cutoff$type=='Inverse SP']]))
print(ggplot(df.ex,aes(value)) + geom_density() +
    geom_vline(data=df.cutoff,aes(xintercept=value),linetype='dashed') + 
    geom_ribbon(data=df.dens,aes(x=x,ymin=0,ymax=y),alpha=0.2) +
    geom_text(data=df.cutoff,aes(x=value,y=y,label=label),size=4,hjust=-0.3) +
    facet_wrap(~type,scales='free') +
    labs(x='AUC over time (EGFR -> TFs)',y='Density')
)
@

% ----------------------------------------------------------------------------

<<label=receptor_ts,echo=False,fig=True,width=10,height=4,eval=True>>=
...
df <- data.frame(receptor=df.integral.sp$receptor,sp.auc=df.integral.sp$V1,sp.pval=df.integral.sp$pval,mfpt.auc=df.integral.mfpt$V1,mfpt.pval=df.integral.mfpt$pval)
df$group <- 'Other'
df$group.num <- '0'
...
df <- df[order(df$group.num,df$sp.auc,decreasing=T),]
df <- within(df, receptor <- factor(receptor, 
    levels = df$receptor))
df <- within(df, group <- factor(group, 
    levels = df$group))
#df$y1 <- NA
#df$y1[df$sp.pval<0.1] <- df$sp.auc[df$sp.pval<0.1] + 0.1
#df$y2 <- NA
#df$y2[df$sp.pval<0.05] <- df$sp.auc[df$sp.pval<0.05] + 0.5
#df$y3 <- NA
#df$y3[df$sp.pval<0.01] <- df$sp.auc[df$sp.pval<0.01] + 1
df$y1 <- NA
df$y1[df$mfpt.pval<0.1] <- df$mfpt.auc[df$mfpt.pval<0.1] + 0.2
df$y2 <- NA
df$y2[df$mfpt.pval<0.05] <- df$mfpt.auc[df$mfpt.pval<0.05] + 0.4
df$y3 <- NA
df$y3[df$mfpt.pval<0.01] <- df$mfpt.auc[df$mfpt.pval<0.01] + 0.6

print(ggplot(df,aes(receptor,mfpt.auc,fill=group)) + 
#print(ggplot(df,aes(receptor,-log10(pval))) + 
    #geom_point(aes(receptor,y1),shape='*',size=5) + 
    #geom_point(aes(receptor,y2),shape='*',size=5) + 
    #geom_point(aes(receptor,y3),shape='*',size=5) + 
    #geom_line(aes(colour=receptor)) +
    ...
    labs(x='Receptors',y='AUC',title='Inverse MFPT\n') +
    theme(axis.text.x=element_text(angle=90,size=7),legend.position=c(0.5,1.1),
...
<<label=fit_diff,echo=False,fig=False,cache=True,eval=True>>=
...
<<label=fit_hist,echo=False,fig=True,eval=False>>=
res <- hist(-log10(p.adjust(pval,'BH')), plot=F)
df <- data.frame(x=res$mids, y=res$counts, type='q-value')

print(ggplot(df,aes(x,y)) + 
    geom_bar(stat='identity',fill='gray',colour='black') +
    labs(x=expression(paste(-log[10]~italic(q),'-value')),
        y='Count',title='Temporal differential expression of H838 genes\n') 
)
@
...
ctrl.df <- list(dat=ctrl[order(pval)[1:1000],], cond='homo', tpoint=tpoint)
treated.df <- list(dat=treated[order(pval)[1:1000],], cond='hetero', tpoint=tpoint)
df.ex <- plot.full.reduced(ctrl.df, treated.df, '~/data/hpaec/h838_fit_full_reduced.pdf')
@

% ----------------------------------------------------------------------------

<<label=gage_diff,echo=False,fig=False,cache=False,eval=False>>=
# GAGE
library(gage)
library(gageData)
data(go.gs)
...
data(egSymb)
library(xtable)
go.gs.sym <- lapply(go.gs, eg2sym)
kegg.gs.sym <- lapply(kegg.gs, eg2sym)

mSigDB <- readLines("~/data/MSigDB/c2.all.v3.0.symbols.gmt")
mSigDB <- strsplit(mSigDB, "\t")
names(mSigDB) <- sapply(mSigDB, function(x) x[1])
mSigDB <- sapply(mSigDB, function(x) x[3:length(x)])

#test with direction: either up or down
df.diff <- data.frame(diff=-log10(pval))
rownames(df.diff) <- ctrl$Gene
res <- gage(df.diff, gsets = mSigDB, ref = NULL, samp = 1, 
    compare='paired', saaTest=gs.KSTest, same.dir=T)
df <- as.data.frame(res$greater)
df <- subset(df[,c('p.val','set.size')], p.val<0.01)
rownames(df) <- sapply(rownames(df), function(x) substr(x,1,40))
tbl <- xtable(df, caption='Significantly ($p < 0.01$) upregulated gene sets in heterogeneous coculture compared to homogeneous coculture.', display=c('s','e','d'))
print(tbl, tabular.environment = "longtable", 
    add.to.row=list(as.list(seq(1,nrow(df),2)),rep("\\rowcolor{Gray} ",
    length(seq(1,nrow(df),2)))), include.rownames=T, 
    file='~/data/hpaec/h838_table_gage_diff_up.tex',
    caption.placement='top')
#df <- as.data.frame(res$less)
#df <- subset(df[,c('p.val','set.size')], p.val<0.01)
#tbl <- xtable(df, caption='Significantly ($p < 0.01$) downregulated gene sets in heterogeneous coculture compared to homogeneous coculture.', display=c('s','e','d'))
...
#    add.to.row=list(as.list(seq(1,nrow(df),2)),rep("\\rowcolor{Gray} ",
#    length(seq(1,nrow(df),2)))), include.rownames=T, 
#    file='~/data/hpaec/h838_table_gage_diff_down.tex',
#    caption.placement='top')
@

% ----------------------------------------------------------------------------

<<label=ts_heatmap,echo=False,fig=True,width=7,height=7,eval=False>>=
...
#mycolor <- rev(brewer.pal(11,'RdBu'))
qval <- p.adjust(pval, 'BH')
final <- treated[qval<0.01,-1]
rownames(final) <- treated$Gene[qval<0.01]
names(final) <- paste(tpoint,'h')
#plot.ts <- function(final) {
    dd.col <- as.dendrogram(hclust(dist(final)))
    ...
    with(df, levels(term))
    df <- melt(df)
    p1 <- ggplot(df,aes(variable,term)) + 
    ...
    p3 <- ggplot(segment(ddata_y)) + 
        ...
        coord_flip() + theme_none + #ylab('\n\n\n') +
        scale_x_discrete(expand = c(0, 0)) +
        scale_y_reverse()

    grid.newpage()
    print(p1, vp=viewport(0.8, 1, x=0.6, y=0.5))
    print(p3, vp=viewport(0.2, 0.94, x=0.1, y=0.5))
#}
#plot.ts(df.ts)

#res <- hclust(dist(final))
#tree <- cutree(res, 2)
#sample <- unlist(mget(names(tree[tree==1]),org.Hs.egSYMBOL2EG,ifnotfound=NA))
#universe <- unlist(mget(as.character(treated$Gene),org.Hs.egSYMBOL2EG,
#    ifnotfound=NA))
#params <- new("GOHyperGParams", geneIds = sample, 
#    universeGeneIds = universe, annotation = "org.Hs.eg", ontology = "BP", 
#    pvalueCutoff = 0.05, conditional = FALSE, testDirection = "over")
#go_over <- hyperGTest(params)
#stat <- summary(go_over)
