...
<<label=ts_example,echo=False,fig=True,width=8,height=6,eval=True>>=
goi <- c('IL6','SAA2','CCL8','IL8','CXCL1','CXCL2')
df <- subset(all.dat, gene %in% goi)
df$gene <- factor(df$gene, levels=goi)
...
print(ggplot(df, aes(time,val)) + 
    ...
    theme(legend.position=c(0.9,0.95),legend.key=element_blank(),
        legend.background=element_blank())
)
#all.param <- ddply(all.dat, .(gene), lrtest.replicate)
#gene <- toupper(as.vector(all.param$gene))
