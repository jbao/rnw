...
<<label=sitar_res,echo=False,fig=True,cache=True,eval=True>>=
# convert symbol to refseq id
entrez.id <- unlist(mget(as.character(df.uptime$gene),org.Hs.egSYMBOL2EG,
    ifnotfound=NA))
entrez.id <- entrez.id[!is.na(entrez.id)]
ref.id <- unlist(mget(entrez.id,org.Hs.egREFSEQ))
ref.id <- ref.id[grep('NM',ref.id)]
symbol <- unlist(mget(unlist(mget(ref.id,org.Hs.egREFSEQ2EG)),org.Hs.egSYMBOL))
df.symbol.refseq <- data.frame(symbol=symbol,refseq=ref.id)
df.symbol.refseq$phase <- df.uptime$phase[match(df.symbol.refseq$symbol,
    df.uptime$gene)]

# read sitar res
motif2symbol <- read.delim('~/data/TRANSFAC/Motif_Symbol.txt') 
wd <- '/home/hbusch/sitar/output/allhuman/'
all.files <- system(paste('find',wd,'-type f -size +0c'),intern=T)
df.symbol.refseq$file <- apply(df.symbol.refseq, 1, function(x) 
    paste(wd,x[2],'.csv',sep='') %in% all.files)
df.symbol.refseq <- subset(df.symbol.refseq, file)
sitar.res <- foreach(i=1:nrow(df.symbol.refseq), .combine='rbind') %dopar% {
    res <- read.delim(paste(wd,df.symbol.refseq$refseq[i],'.csv',sep=''))
    idx <- which(res$Score>1)
    df <- data.frame(gene=df.symbol.refseq$symbol[i], 
        tf=motif2symbol$Symbol[match(res$TFname[idx],motif2symbol$Motif)],
        score=res$Score[idx], phase=df.symbol.refseq$phase[i])
}
sitar.res <- subset(sitar.res, !is.na(tf))
@

% ----------------------------------------------------------------------------

<<label=sitar_plot,echo=False,fig=True,eval=True>>=
# t-test of sitar scores between early and late genes
pval.early.late <- function(df, type) {
    df.early <- subset(df, phase=='early')
    df.early <- df.early[!duplicated(df.early$score),]
    df.late <- subset(df, phase=='late')
    df.late <- df.late[!duplicated(df.late$score),]
    if (nrow(df.early)>1 & nrow(df.late)>1) {
        res <- t.test(df.early$score,df.late$score)
        if (type=='stat')
...
#unique.tf <- unique(sitar.res$tf)
#for (i in 1:length(unique.tf)) {
#    df <- subset(sitar.res, tf==unique.tf[i])
#    
#    if (length(score.early)>1 & length(score.late)>1)
#        t.test(score.early,score.late)$statistic
#}
tstat <- ddply(sitar.res, .(tf), function(x) pval.early.late(x,'stat'))
pval <- ddply(sitar.res, .(tf), function(x) pval.early.late(x,'pval'))
...
#    geom_text(data=subset(df.t,V1<0.1),aes(label=tf),size=3,hjust=-0.2) +
...
print(ggplot(df,aes(abs(t),-log10(tf.diff.qval))) + geom_point() +
...
<<label=ts,echo=False,fig=True,eval=False>>=
#dat <- dat[1:12,]
pdf('~/data/hdf/hdf-fit_full_reduced.pdf',8,11)
#for (i in 1:ceiling(nrow(dat)/12)) {
for (i in 1:100) {
    slice <- seq((i-1)*12+1,i*12)
    slice <- slice[!(slice > nrow(ctrl))]
    df.ctrl <- melt(ctrl[slice,])
    df.ctrl <- transform(df.ctrl, cond='Control')
    df.treated <- melt(treated[slice,])
    df.treated <- transform(df.treated, cond='NHK')
    df <- rbind(df.ctrl, df.treated)
    df$variable <- as.numeric(as.vector(df$variable))
    df <- within(df, Gene <- factor(Gene, levels = ctrl$Gene[slice]))
    with(df, levels(Gene))
    df <- within(df, cond <- factor(cond, levels = c('NHK','Control')))
    with(df, levels(cond))
    
    # logistic fit
    core <- cbind(ctrl[slice,-1],treated[slice,-1])
    ...
    #df.param <- apply(df.fit,2,function(x)xfit[which.max(abs(diff(x)/diff(xfit)))])
    df.param <- apply(cbind(ctrl[slice,-1],treated[slice,-1]),1,lrtest)
    #stop()
    ...
    fit.reduced <- transform(fit.reduced, Gene=rep(symbol[slice],each=length(ctrl.tpoint)))
    fit.reduced <- within(fit.reduced, Gene <- factor(Gene, levels = symbol[slice]))
    with(fit.reduced, levels(Gene))
    
    fit.ctrl <- transform(fit.ctrl, time=ctrl.tpoint)
    fit.ctrl <- melt(fit.ctrl, id.vars='time')
    fit.ctrl <- transform(fit.ctrl, Gene=rep(symbol[slice],each=length(ctrl.tpoint)))
    ...
    fit.treated <- transform(fit.treated, Gene=rep(symbol[slice],each=length(treated.tpoint)))
    fit.treated <- within(fit.treated, Gene <- factor(Gene, levels = symbol[slice]))
    with(fit.treated, levels(Gene))
    
    print(ggplot(df, aes(variable,value)) + geom_point(aes(colour=cond)) + 
        ...
        geom_text(data=df.param, aes(x=3,y=6.5,label=label),size=3,parse=T) +
        facet_wrap(~Gene, ncol=3) + 
        scale_colour_brewer('',palette='Set1') +
        ylim(-4,7) +
        xlab('time (h)') + ylab('log2 fold expression') +
...
<<label=mfpt,echo=False,fig=False,cache=True,eval=False>>=
...
idx <- grep('NFK', names(mSigDB))
nfkb.target <- unique(unlist(sapply(idx, function(x) mSigDB[[x]])))

human <- hdf5load('../RandomWalk/biogrid/BIOGRID-ORGANISM-Homo_sapiens-3.1.89-MFPT.hdf5',load=F)
human.map <- hdf5load('../RandomWalk/biogrid/BIOGRID-ORGANISM-Homo_sapiens-3.1.89.tab2.txt.hdf5',load=F)
...
hppi <- hdf5load('/home/jbao/data/network/hppi/hppi2-directed-MFPT.hdf5', load=F)
hppi.map <- hdf5load('/home/jbao/data/network/hppi/hppi2_directed_map.hdf5', load=F)
@

% ----------------------------------------------------------------------------

<<label=hippie,echo=False,fig=False,cache=True,eval=False>>=
#hippie <- hdf5load('/home/jbao/data/network/hippie/HIPPIE-current-MFPT.hdf5', load=F)
#hippie.map <- hdf5load('/home/jbao/data/network/hippie/HIPPIE-current.mitab.txt.hdf5', load=F)
hippie <- hdf5load('/home/jbao/data/network/hippie/HIPPIE-current_cutoff0-MFPT.hdf5', load=F)
hippie.map <- hdf5load('/home/jbao/data/network/hippie/HIPPIE-current_cutoff0_map.hdf5', load=F)
hippie.net <- read.graph('/home/jbao/data/network/hippie/HIPPIE-current_cutoff0.xml', 'graphml')
#hippie <- hdf5load('/home/jbao/data/network/hippie/HIPPIE-current_cutoff0.7-MFPT.hdf5', load=F)
#hippie.map <- hdf5load('/home/jbao/data/network/hippie/HIPPIE-current_cutoff0.7_map.hdf5', load=F)
@
...
<<label=tfbs,echo=False,fig=False,cache=True,eval=False>>=
# GAGE
library(gage)
data(kegg.gs)
data(egSymb)
...
%\begin{figure}[ht]
%\begin{center}
<<label=tf_volcano,echo=False,fig=True,cache=True,eval=False>>=
ctrl <- read.delim('~/data/hdf/hdf_ctrl_16091.tsv')
treated <- read.delim('~/data/hdf/hdf_nhk_16091.tsv')
diff <- read.delim('~/data/hdf/hdf_diff_16091.tsv')
...
<<label=tf_seq,echo=False,fig=False,eval=False>>=
...
#tf.sig[i] <- 'TSHZ1'
qpcr.gene <- read.delim('~/data/hdf/hdf_qpcr_pval.tsv')$gene
tf.sig <- c(as.character(qpcr.gene), 'TGFB2', 'TGFB3')
...
for (i in 1:length(idx)) {
    ...
    if (symbol != '') {
        seq.file <- paste('~/data/hdf/qpcr_seq/',symbol,'.fa',sep='')
