...
<<label=degree_response,echo=False,fig=True,eval=True>>=
org <- 'yeast'
org.dir <- 'yeast'
if (org == 'yeast') {
    type <- c('alpha', 'cdc15', 'cdc28', 'elu')
    size <- 6178
    dat <- read.delim('~/data/yeast/yeast_alpha.dat', sep='|')
    #ppi <- read.graph('/home/jbao/data/network/biogrid/biogrid_yeast_3.1.93.xml', 'graphml')
    elist <- read.delim('~/gnw/src/ch/epfl/lis/gnwgui/rsc/net/yeast_transcriptional_network_Balaji2006.tsv', header=F)
    ppi <- graph.data.frame(elist[,1:2], directed=F)
} else {
    type <- c('cold', 'heat', 'oxidative', 'lactose')
    size <- 4400
    dat <- read.delim('~/data/ecoli/GSE20305_cold.dat', sep='|')
    elist <- read.delim('~/gnw/src/ch/epfl/lis/gnwgui/rsc/net/ecoli_transcriptional_network_regulonDB_6_2.tsv', header=F)
    ppi <- graph.data.frame(elist[,1:2], directed=F)
}

all.dat <- NULL
for (t in 1:length(type)) {
    ts <- read.delim(paste('~/data/',org.dir,'/',org,'_',type[t],'.dat',sep=''), sep='|')
    pval <- read.delim(paste('~/data/',org.dir,'/mds/pval_',org,'_',type[t],'_',size,'.dat',sep=''), header=F)$V1
    common <- intersect(V(ppi)$name, ts$Gene)
    ppi.idx <- match(common, V(ppi)$name)
    pval.idx <- match(common, ts$Gene)
    deg <- degree(ppi, ppi.idx)
    pval <- pval[pval.idx]
    all.dat <- rbind(all.dat, data.frame(degree=deg,response=-log10(pval),
        cond=type[t], gene=common))
}

print(ggplot(all.dat, aes(response,degree,colour=cond)) + 
    geom_point() +
    ...
    labs(x='Response',y='Degree',title=expression(atop(paste('Time resolved response of'~italic(S.cerevisiae)),paste('to synchronization')))) +
    #labs(x='Response',y='Degree',title=expression(atop(paste('Time resolved response of'~italic(E.coli)),paste('to stress')))) +
    theme(legend.position=c(0.9,0.85),legend.key=element_blank(),
...
<<label=go_data,echo=False,fig=False,cache=True,eval=True>>=
res <- NULL
for (t in 1:length(type)) {
    sig.gene <- all.dat$gene[all.dat$cond==type[t]&all.dat$response>-log10(0.05)]
    #sig.gene <- all.dat$gene[all.dat$cond==type[t]&all.dat$response < -log10(0.95)]
    # GO enrichment
    if (org == 'GSE20305') {
        sample <- unlist(mget(as.character(sig.gene), 
            org.EcK12.egSYMBOL2EG, ifnotfound=NA))
        universe <- unlist(mget(as.character(dat$Gene),
            org.EcK12.egSYMBOL2EG, ifnotfound=NA))
        #universe <- dat$Gene
        params <- new("GOHyperGParams", geneIds = sample, 
            universeGeneIds = universe, annotation = "org.EcK12.eg", ontology = "BP", 
            pvalueCutoff = 0.05, conditional = FALSE, testDirection = "under")
    }
    else {
        universe <- dat$Gene
        params <- new("GOHyperGParams", geneIds = sig.gene, 
            universeGeneIds = universe, annotation = "org.Sc.sgd", ontology = "BP", 
    ...
    if (t == 1)
        ...
        common <- intersect(common, stat$Term)
    tmp <- stat[match(common,stat$Term),]
    tmp$cond <- type[t]
    res <- rbind(res, tmp)
}
res <- res[res$Term %in% common,]
...
<<label=go,echo=False,fig=True,eval=True,width=7,height=8>>=
res <- subset(res, level==4)
final <- reshape(res[,c('cond','Term','Pvalue')], v.names='Pvalue', 
    idvar='Term', timevar='cond', direction='wide')
rownames(final) <- final$Term
final <- final[,-1]
dd.col <- as.dendrogram(hclust(dist(t(final))))
col.ord <- order.dendrogram(dd.col)

df <- final[,col.ord]
names(df) <- sub('Pvalue\\.','',names(df))
df.m <- melt(as.matrix(df))
df.m <- within(df.m, X2 <- factor(X2, levels = names(df)))
df.m$X1 <- sapply(df.m$X1, function(x) strwrap(x,width=40)[1])
mycolor <- gray.colors(128,0.9,0)
p1 <- ggplot(df.m,aes(X2,X1)) + 
    #geom_tile(aes(fill=-log10(value))) +
    geom_tile(aes(fill=-log10(value))) +
    scale_fill_gradientn(colours=mycolor) +
    guides(fill = guide_colorbar(barwidth = 0.5, barheight = 10, 
        title.hjust=1.6, title.vjust=0,
...
grid.newpage()
print(p1, vp=viewport(1, 0.9, x=0.5, y=0.45))
print(p2, vp=viewport(0.4, 0.1, x=0.65, y=0.95))
@
